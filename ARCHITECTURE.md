...
| Editor guardrails | `addons/nascentsoul/runtime/zone_configuration_warnings.gd` | Keeps Inspector-facing zone/layout/space-model compatibility warnings out of `Zone.gd` while surfacing setup mistakes before runtime |
...
| Input workflow | `addons/nascentsoul/runtime/zone_input_service.gd`, `zone_input_binding_registry.gd`, `zone_input_pointer_flow.gd`, `zone_input_selection_controller.gd` | `ZoneInputService` stays as the orchestration facade while binding lifecycle, pointer/long-press flow, and selection behavior each stay in focused helpers |
