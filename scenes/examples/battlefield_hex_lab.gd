...
@export var battlefield_width_limits_desktop: Vector2 = Vector2(520.0, 560.0)
@export var battlefield_width_limits_compact: Vector2 = Vector2(500.0, 540.0)
@export var battlefield_width_limits_mobile: Vector2 = Vector2(480.0, 520.0)
...
@export_range(0.1, 1.0, 0.01) var battlefield_min_scale: float = 0.82
...
@onready var _source_zone: Zone = $RootMargin/RootVBox/ContentRow/SourceColumn/SourcePanel/HexSourceZone as Zone
@onready var _battlefield_zone: BattlefieldZone = $RootMargin/RootVBox/ContentRow/BattlefieldColumn/BattlefieldPanel/HexBattlefieldZone as BattlefieldZone
...
func _ready() -> void:
	_space_model = ExampleSupport.get_zone_space_model(_battlefield_zone) as ZoneHexGridSpaceModel
...
func _apply_hex_grid_layout(space_model: ZoneHexGridSpaceModel, panel_size: Vector2, base_cell_size: Vector2, padding: Vector2) -> void:
	if space_model == null:
		return
	var columns: int = max(space_model.columns, 1)
	var rows: int = max(space_model.rows, 1)
	var resolved_base_cell := Vector2(max(base_cell_size.x, 1.0), max(base_cell_size.y, 1.0))
	var base_width: float = resolved_base_cell.x * (float(columns) + 0.5)
	var base_height: float = resolved_base_cell.y * (1.0 + 0.75 * float(max(rows - 1, 0)))
	var inner_width: float = max(1.0, panel_size.x - padding.x * 2.0)
	var inner_height: float = max(1.0, panel_size.y - padding.y * 2.0)
	var scale: float = clamp(min(inner_width / base_width, inner_height / base_height), battlefield_min_scale, 1.0)
	space_model.cell_size = Vector2(floor(resolved_base_cell.x * scale), floor(resolved_base_cell.y * scale))
