...
func _test_square_battlefield_direct_place() -> void:
	...
	_check(source_zone.move_item_to(spark, battlefield, target), "card should move from card zone into square battlefield")
	await _settle_frames(2)
...
func _test_square_occupancy_rejects_second_item() -> void:
	...
	_check(source_zone.move_item_to(alpha, battlefield, target), "first item should occupy a free battlefield cell")
	await _settle_frames(2)
	_check(not source_zone.move_item_to(beta, battlefield, target), "second item should be rejected from an occupied battlefield cell")
	await _settle_frames(2)
...
func _test_hex_battlefield_direct_place() -> void:
	...
	_check(source_zone.move_item_to(ember, battlefield, target), "card should move from card zone into hex battlefield")
	await _settle_frames(2)
...
func _test_spawn_piece_transfer_rule() -> void:
	...
	_check(source_zone.move_item_to(sigil, battlefield, target), "summon battlefield should accept a card and spawn a piece")
	await _settle_frames(2)
...
func _test_battlefield_piece_reposition() -> void:
	...
	await _settle_frames(2)
	_check(battlefield.move_item_to(piece, battlefield, moved_target), "piece should be movable within the same battlefield zone")
	await _settle_frames(2)
...
func _test_piece_can_move_between_piece_battlefields() -> void:
	...
	await _settle_frames(2)
	_check(left_zone.move_item_to(piece, right_zone, ZonePlacementTarget.square(1, 0)), "piece should be movable into another piece-friendly battlefield zone")
	await _settle_frames(2)
...
func _test_modes_restrict_piece_backflow() -> void:
	...
	await _settle_frames(2)
	_check(source_zone.move_item_to(direct_card, direct_zone, ZonePlacementTarget.square(0, 0)), "card should enter Direct Place battlefield")
	await _settle_frames(2)
	...
	_check(source_zone.move_item_to(summon_card, summon_zone, ZonePlacementTarget.square(0, 0)), "card should enter Spawn Piece battlefield")
	await _settle_frames(2)
	...
	_check(not summon_zone.move_item_to(spawned_piece, source_zone, ZonePlacementTarget.linear(source_zone.get_item_count())), "spawned piece should not move back into the Cards zone")
	await _settle_frames(2)
	_check(not summon_zone.move_item_to(spawned_piece, direct_zone, ZonePlacementTarget.square(1, 0)), "spawned piece should not move into Direct Place because that zone keeps card objects")
	await _settle_frames(2)
