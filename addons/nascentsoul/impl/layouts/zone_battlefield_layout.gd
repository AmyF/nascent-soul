...
func calculate(items: Array[Control], container_size: Vector2, ghost_item: Control = null, ghost_hint = null, runtime = null) -> Array[ZonePlacement]:
	...
	for index in range(render_items.size()):
		...
		if item == ghost_item:
			...
			target = runtime.get_item_target(item)
		var item_size = resolve_item_size(item)
		var position = Vector2.ZERO
		var scale = Vector2.ONE
		var z_index = index
		if runtime != null:
			var target_size = runtime.resolve_target_size(target)
			...
			position = runtime.resolve_target_position(target, container_size, scaled_size)
