...
## Returns the placement target currently under the pointer, preserving both global and local positions when possible.
...
## Returns a canonical target for target within this space model.
...
## Returns the default target for inserting _item into context, optionally using _hint to preserve caller intent.
...
## Returns the canonical target representing item's current logical placement for rendering and queries.
...
## Returns a layout-specific hint derived from target for preview ghost placement.
func resolve_layout_hint(target: ZonePlacementTarget):
	return target.get_linear_index() if target != null else -1

## Returns the local top-left position where _target should render inside the current container.
...
## Returns the logical size of _target for previews and targeting visuals.
func resolve_target_size(_context: ZoneContext, _target: ZonePlacementTarget) -> Vector2:
	return Vector2.ZERO

## Returns a global anchor point for target used by overlays and placement feedback.
...
## Returns whether target is usable in this space model for the current context.
func is_target_valid(_context: ZoneContext, target: ZonePlacementTarget) -> bool:
	return target != null and target.is_valid()

## Returns whether two targets should be treated as the same logical location.
func targets_match(a: ZonePlacementTarget, b: ZonePlacementTarget) -> bool:
	if a == null or b == null:
		return false
	return a.matches(b)

## Returns the first currently available target for item.
